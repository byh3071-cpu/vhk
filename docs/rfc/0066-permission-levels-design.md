...
부작용이 생기려면 사람이 두 번 켜야 하므로 "자동으로는 아무것도 켜지지 않는다" 는 T4 정신은 유지된다.
`record` 만 켠 상태에서 집행 경로는 코드에 도달하지 않는다 — §7.5 의 부작용 단언은
**아무 플래그도 없는 상태**를 기준으로 유지한다.
...
켤 때 `last != null` 이 되어 케이스 0을 밟지 않고, 케이스 2가 적용돼 현행 단계가 유지된다.
데드락이 원천 제거된다.
...
| Q5 | **off일 때 원장 0줄 순환(§7.2)을 어떻게 끊나** | (A) 끊지 않음 — T4 우선, 켜기 판단은 `vhk stats`로 (B) `--record` 옵트인 신설 (C) 판정 이력만 `.vhk/` 비추적 경로에 별도 보관 | **확정: (B) 옵트인 기록** (2026-08-20 — 기본값 뒤집음). (A)의 근거였던 대안 지표가 실측상 부재였고, (A)는 순환이 아니라 승급 불가 **데드락**이다. 상세는 §7.2 |
