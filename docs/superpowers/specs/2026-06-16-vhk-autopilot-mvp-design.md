...
- CLI `vhk auto`, MCP `vhk_auto`, private-rules-repository core-ruleset 규약, sync 배포.
...
| 배포 | CLI `vhk auto`(등록 7항 + MCP 30→31) · MCP `vhk_auto` · private-rules-repository core-ruleset 규약(inject_core_rules) · sync 자동설치. |
