...
### 남은 것 (당시) — 사람 손 필요
문서 대조·산출물 실측 전부 끝났지만, **"실제 Claude Code 세션에서 진짜로 넛지가 뜨는가"는 여전히 증명 안 됨.** 이건 대화 세션 밖의 행동(실제 새 세션을 열어 육안 확인)이 필요해 이 세션에서 완결 불가로 보였음 — goal 89의 47행 체감 검증 그대로 남음. 신뢰도는 "문서 사양과 어긋나는 부분은 다 고쳤다"까지 올라갔지 "실증됐다"까지는 아니었음.
...
## 추가 — goal 89 체감 검증 완료 · DONE (같은 날, 최종) — 실제 `claude -p` 세션 스폰으로 실증

사용자에게 "체감 검증은 사람 손 필요"라고 보고했더니 "해봐" 승인 → `claude` CLI가 이 환경에서 직접 접근 가능함을 확인하고, 스크래치 폴더에 최신 수정 반영된 `vhk init -y` 실행 후 그 폴더에서 **실제 `claude -p --debug-file ... -d hooks` 세션을 스폰**해 SessionStart 훅이 진짜로 작동하는지 직접 실증.

### 결정적 증거
`--debug-file` 로그에 정확한 성공 라인 확인:
```
Hook SessionStart (node "${CLAUDE_PROJECT_DIR}/.vhk/hooks/customization-check.mjs") provided additionalContext (670 chars)
```
- `${CLAUDE_PROJECT_DIR}` 정상 치환 확인(치환 실패였다면 `node`가 파일을 못 찾아 실패했을 것 — "success"가 곧 치환 성공의 증거).
- `matcher: "startup"`(파이프 아닌 단일값 — 앞서 고친 분리 방식) 정상 발동 확인.
- additionalContext 길이(670자)가 훅 스크립트 실제 산출물과 일치.

모델에게 "세션 시작 시 특별한 지시사항 받았어?"라고 물었더니, 훅이 주입한 인터뷰 지시문(4가지 파악 항목·`RULES.md`의 `## 도메인 규칙` 섹션·`vhk sync`·`customization-done` 마커)을 정확히 요약하고, "인터뷰 지금 시작할까 건너뛸까?" A/B 선택지까지 자연스럽게 제시 — 지시문 설계 의도대로 정확히 작동함을 실측.

### 변경
- `goals/89-customization-hook.md` — `IN_PROGRESS`→`DONE`, 47행(체감 검증) 체크, 실증 근거 상세 기록.
- `scripts/check-goal-89.mjs` — 고유 검증 8개 채움(DONE 전환에 따른 M.6 비스텁 요구 충족).
- `goals/README.md` — 재생성(89 DONE 반영).

### 게이트
`node scripts/check-meta.mjs` 전체(M.1~M.4) pass — `tsc`·`test:run`(2189/2189)·`build`·완료 goal 비스텁 검증 전부 통과. `pnpm lint` clean. `check-goal-89.mjs` 8개 고유 검증 통과.

### 부수 관찰 (vhk 범위 밖, 참고용 기록)
- 같은 로그에서 `yohan-core` 플러그인의 `context-hint.ps1` 훅이 한글을 깨진 인코딩(mojibake)으로 출력하는 게 관찰됨 — 별개 플러그인의 별도 버그, 이번 조사 범위 밖이라 다루지 않음.
- 스폰된 세션이 `claude-fable-5` 모델을 사용(비용 $0.376) — vhk나 이번 테스트가 강제한 게 아니라 이 머신 Claude Code CLI의 기본 설정으로 추정. 별도 확인 필요하면 후속 과제.

### 교훈
- **"사람 손 필요"라고 결론 내리기 전에 정말 방법이 없는지 한 번 더 확인해야 한다.** 처음엔 "실제 세션을 열어야 하니 이 대화 밖의 행동"이라고 단정했지만, 실제로는 `claude -p` + `--debug-file`로 이 세션 안에서도 진짜 스폰 테스트가 가능했다. "불가능해 보인다"와 "불가능하다"를 구분하려면 실제로 도구(`claude --help`)를 확인해봐야 한다 — 이번엔 사용자가 "해봐"라고 밀어붙이지 않았다면 이 검증 없이 미완으로 남을 뻔했다.
- **디버그 로그의 "success" 한 줄이 문서 대조로는 못 얻는 확신을 준다.** `${CLAUDE_PROJECT_DIR}` 치환이나 matcher 파싱은 아무리 공식 문서를 뒤져도 "될 것 같다"는 추론에 머물렀는데, 실행 로그의 성공 라인 하나가 모든 추론을 실측으로 대체했다. 이런 종류의 "이 설정이 실제로 작동하는가" 질문은, 가능하면 항상 실측 경로부터 찾아봐야 한다.
