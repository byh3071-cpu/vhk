...
- VHK 정본 저장소의 manifest·생성 번들 중 하나라도 없거나 일반 파일이 아니거나, manifest·정본
  본문·생성 번들이 서로 다르면 소비자 프로젝트로 강등하지 않고 손상된 정본으로 차단한다.
- 실행 시 발견한 정본·투영 경로의 symlink·junction은 따라가지 않는다. 관리본 갱신은 기존 경로를
  `.vhk/backups/<id>/`로 먼저 원자 이동한 뒤 새 경로를 배타 생성한다. 계획 뒤 파일 내용이 바뀌었으면
  원래 경로를 복원하고 충돌로 끝낸다. 갱신 성공 때만 이전 관리본을 독립 백업으로 유지하며, 실패 때
  활성 경로를 복구하면서 백업 이름을 제거했으면 존재하지 않는 백업 경로를 안내하지 않는다.
  신규 경로의 완성본·비덮어쓰기 보장은 같은 파일시스템 hard link를 사용한다. 이를 지원하지 않는
  파일시스템에서는 비원자 copy로 낮추지 않고 실패한다. 실제 I/O 실패는 사용자 충돌 경고로 숨기지 않는다.
...
- 로컬에서 저장소 경로 자체를 적대적으로 교체할 수 있는 프로세스는 이미 코드·테스트·패키지 입력도
  바꿀 수 있다. VHK는 실행 시 링크 검사와 원본 이동 백업을 제공하지만, 악성 로컬 writer 격리는
  운영체제 sandbox와 별도 worktree 책임이다.
