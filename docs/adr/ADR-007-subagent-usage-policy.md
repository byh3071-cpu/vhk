...
- **VHK는 외부 서브에이전트 플러그인을 선택적으로 사용할 수 있다.** 역할별 에이전트 정의는 플러그인에 두고 VHK 저장소에 복제하지 않는다.
...
1. **범용 에이전트 정의는 외부 플러그인을 단일소스(SoT)로 유지한다.** VHK 저장소에 복제하지 않는다 — SoT가 2곳이 되면 드리프트다.
...
6. **VHK 전용 에이전트가 필요해지면** 플러그인과 이름이 겹치지 않게 프로젝트 `.claude/agents/`에 추가한다. 현재 명확한 수요는 없어 보류.
...
- **실측 결함 (probe 2026-06-30)**: 외부 critic 에이전트가 읽기 전용 선언과 달리 임의 경로에 파일을 생성했다.
- **추정(미검증)**:
...
- **후속 대응**: critic 호출에는 쓰기·커밋 금지를 명시하고, 다른 역할도 동일 probe로 권한 강제를 확인한다. 필요하면 프로젝트 권한 deny로 구조적으로 차단한다.
