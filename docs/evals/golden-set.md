...
# 자동화 도우미 원본→요약 골든 셋 (Golden Set)

> **TL;DR** — 이 파일은 노션 AI 에이전트 "자동화 도우미"가 수행하는 **원본→요약(source→summary) 처리 프로토콜**의 출력 품질을 회귀 검사(regression test)하기 위한 **골든 셋**(정답 기준 케이스 모음)이다. `vhk check evals`가 이 파일을 읽어 케이스별로 채점한다. 이 문서는 노션 워크스페이스 밖(레포·CI·외부 AI)에서도 단독으로 이해되도록 자기설명형으로 작성됐다.

## 1. 이 파일은 무엇인가
- **평가 대상:** 자동화 도우미가 원본 자료(녹음 / 영상 / URL / 이미지 / 텍스트)를 받아 노션 RESOURCE·SUMMARY로 정리하는 작업의 품질.
- **소비자:** VHK CLI의 `vhk check evals` 명령. (VHK = yohanstudio의 바이브코딩 가드레일 + 로컬 Evals CLI 툴)
...
- **회귀 사용법:** 자동화 도우미의 실제 SUMMARY 산출물을 각 케이스 `input` 시나리오로 돌려 MUST 충족 여부를 확인 → 프로토콜 개정 때마다 이 세트로 안전망 확보.
...
- `input`: 자동화 도우미에게 주어지는 가상 시나리오(평가 투입 상황).
...
| 용어 | 뜻 |
| --- | --- |
| 자동화 도우미 (Noddugi) | yohanstudio의 노션 AI 에이전트 이름(요약 작업 수행 주체). |
