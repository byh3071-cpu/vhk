...
> 성장 루프의 마지막 도미노(패턴→진화)를 닫는 **반자동 진화 v0**. 철학: 자동 학습 OK · 자동 적용 금지 —
> 모든 반영은 diff · 사람 승인 · 되돌리기(undo)를 거친다.

### Added

- **`vhk evolve`** (`src/commands/evolve.ts`) — `.vhk/evolve/queue.json`(version 1) 큐로 패턴 → 룰 후보를 관리.
  - `suggest` — active `avoid` 패턴(Goal 19)에서 결정적 한국어 룰 초안 생성(ML/LLM 0, 같은 입력=같은 출력).
    A1: 같은 후보가 `rejected` 면 재제안 억제 · A2: `pending`/`applied` 면 스킵.
  - `list [--status pending|rejected|applied]` · `apply <id>` · `reject <id>` · `undo`.
  - `apply`: TTY 확인 + diff 표시 + 문구 직접 수정(B2) → RULES.md append → `vhk sync` 재생성. `.bak` 백업으로
    원자적 처리(중간 실패 시 롤백). C1: 미해소 `applied` 있으면 단일-apply 제약으로 차단. B3: RULES.md 중복 룰 감지.
  - `undo`: 최근 apply 1건을 `.bak` 으로 복원 + sync 재실행 + 소스 패턴 `archived → active` 복구.
  - A3: apply 시 큐 항목 `applied` + 소스 패턴 `archived`(Goal 18 status 선순환 재사용). A4: 댕글링 참조 가드.
  - 큐 I/O 는 BOM-safe + 원자적 쓰기(`.tmp` → rename). 트리거는 **수동만**(자동 실행 없음).
- **MCP `evolve-suggest` · `evolve-list`** — MCP 도구 총 **29종**(비대화형, `runVhkCli` 위임). apply/reject/undo 는
  대화형 · 위험작업이라 MCP 제외.

### Note

- **안전 1원칙 유지** — 자동 적용 0. 반영은 항상 사람 승인 + diff + undo. secret 미포함(memory/패턴 파생 → secure scan 관문).
- Evolution Loop 도미노 4종(증거→기억→패턴→진화) 완성. 다음 봉우리는 v3 "VHK Hub"(크로스 프로젝트 학습).

## [2.2.0] - 2026-06-04

> **버전 범프만.** 기능 변화 없음 — `package.json` 버전만 상향. 코드·동작 무변경.
...
> **pattern detection v0 — Evolution Loop 도미노 3 (Goal 19).** active 실패·성공 기억에서 반복을 추출해
> `avoid`/`reinforce` 후보를 만든다. **읽기·제안만 — RULES.md 반영 0**(반영은 Goal 20 evolve). 보수적 임계(기본 3회+),
> ML/LLM/외부 의존 0.

### Added

- **`vhk pattern`** (`src/commands/pattern.ts`) — `memory.json` 의 active `failures`+`successes` 에서 2축 감지.
  - **① 태그 군집** — 같은 버킷 태그 빈도 ≥ 임계 → 후보. **② 키워드 문서빈도** — 신호 텍스트 정규화 후 토큰
    문서빈도 ≥ 임계 → 후보. 임계 기본 **3**(`--min <n>` 조정). failures → `avoid`, successes → `reinforce`.
  - `detect`(`--min`,`--json`) · `list`(`--kind`,`--all`) · `dismiss <n>`(→archived). 별칭 `패턴`, NLP `패턴/반복/되풀이/버릇`.
  - **결정적·멱등** — count desc → signal asc 안정 정렬, 시그니처(`kind:axis:정규화signal`) 병합으로 재실행 시 중복 0.
  - `resolved`/`archived` 기억은 입력 제외(선순환 존중 — 해결된 실수는 다시 안 봄).
...
- **반영 0** — detect 는 RULES.md · 다른 버킷 무변경(회귀 가드). secret 미포함(secure scan 관문).
  한국어 형태소분석 · 의미/임베딩은 제외(v0 = 구조/빈도만).
