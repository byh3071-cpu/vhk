...
- **다음 할 일:** ⓪ **[새 전략 최우선 · RFC 0056] `vhk receipt` MVP** — 진입점 `docs/log/2026-06-22-rfc0056-evidence-receipt.md`. 선결: vhk가 추적파일(`.vhk/events/*.jsonl`) 갱신→늘 dirty→receipt 늘 block 봉인. T1 receipt(기계증거 4종→영수증, 등록 4지점+드리프트 테스트 수용기준) → T2 위조방어 → T3 거짓완료 적발 1건(현 0/8). 90일 단일 성공기준=적발 1건. ① **우선순위 2 measure-first**(현재 최우선) — Recall@5는 `vhk memory eval --init` 사용자 대화형 라벨링 선결(며칠 누적) · diff-cover는 실작업 diff 누적 시 측정 · G50(diff-coverage)도 동일 지점. ② ✅ **[완료 2026-06-23] v2.7.0 npm 발행** (2.6.0→2.7.0 latest, 집 컴퓨터 `npm publish --ignore-scripts`+보안키. 절차: docs/log/2026-06-23-vhk-2.7.0-publish.md). ③ **미완 P2 goal**: 62(docs-diff)·65(precommit-l2 조건부)·73(#276 `vhk check --evals` 미기안). ④ 작은 갭: `.vhk/ops-prompt.md`·`sell-prompt.md` gitignore 미등록(goal 82가 ledger만 다룸) → `.vhk/.gitignore`+`templates/vhk-dir.ts` 보강.
