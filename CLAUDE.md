...
- **다음 할 일:** ⓪ **[2026-06-23 갱신 · 의도 장갑] receipt MVP(#377)·#315 봉인(#370) 머지 완료** → 다음 P0 = **[Goal 87 의도 대조](goals/87-mission-verify-intent-check.md)** (receipt/review가 mission scope/forbidden 대조 — 끊긴 의도루프 잇기, 해자 실현). 진입점 `docs/log/2026-06-23-intent-glove-plan.md`. 정체성 = **의도 장갑**(인간 의도→AI 전달→의도대로 검증, 간판=verify/receipt). 잔여 receipt: T2 위조방어 · T3 거짓완료 적발 1건(현 0/8, 90일 단일 성공기준). ① **우선순위 2 measure-first**(현재 최우선) — Recall@5는 `vhk memory eval --init` 사용자 대화형 라벨링 선결(며칠 누적) · diff-cover는 실작업 diff 누적 시 측정 · G50(diff-coverage)도 동일 지점. ② ✅ **[완료 2026-06-23] v2.7.0 npm 발행** (2.6.0→2.7.0 latest, 집 컴퓨터 `npm publish --ignore-scripts`+보안키. 절차: docs/log/2026-06-23-vhk-2.7.0-publish.md). ③ **미완 P2 goal**: 62(docs-diff)·65(precommit-l2 조건부)·73(#276 `vhk check --evals` 미기안). ④ 작은 갭: `.vhk/ops-prompt.md`·`sell-prompt.md` gitignore 미등록(goal 82가 ledger만 다룸) → `.vhk/.gitignore`+`templates/vhk-dir.ts` 보강.
