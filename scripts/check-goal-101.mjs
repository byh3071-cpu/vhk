#!/usr/bin/env node
// scripts/check-goal-101.mjs — 자동 생성 (vhk goal sync).
...
if (existsSync('.vhk/HARD_STOP')) {
  console.log('🛑 .vhk/HARD_STOP detected — refusing to run goal 101 gate.')
  process.exit(1)
}
...
let pass = true
const gate = (label, ok) => { console.log('[goal 101] ' + label + ': ' + (ok ? '✓' : '✗')); if (!ok) pass = false }
...
must(/autonomy-log/.test(repoSkill ?? ''), 'repo vhk-auto/SKILL.md 가 autonomy-log 배선')

const roadmap = read('docs/roadmap/autonomy-evolution.md')
must(
  /SoT|복제본|replica/i.test(roadmap ?? ''),
  'roadmap 에 스킬 SoT=레포 / 글로벌=복제본 안내'
)
...
if (pass) { console.log('✅ goal 101 gate passes'); process.exit(0) }
console.log('❌ goal 101 gate failed'); process.exit(1)
