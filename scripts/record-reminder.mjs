...
// 턴 종료 시점에 미커밋 실질 코드변경이 있는데 세션 dev log(docs/log/<오늘>-*.md)가
...
function main() {
  ...
  if (!existsSync('docs/log') && !existsSync('.git')) return
  ...
  const msg = [
    `[record-reminder] 미커밋 코드변경 ${code.length}건, 오늘자 dev log(docs/log/${today}-*.md) 없음.`,
