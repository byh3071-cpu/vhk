#!/usr/bin/env node
// scripts/check-goal-104.mjs — 자동 생성 (vhk goal sync).
...
if (existsSync('.vhk/HARD_STOP')) {
  console.log('🛑 .vhk/HARD_STOP detected — refusing to run goal 104 gate.')
  process.exit(1)
}
...
let pass = true
const gate = (label, ok) => { console.log('[goal 104] ' + label + ': ' + (ok ? '✓' : '✗')); if (!ok) pass = false }
...
// ─── goal 104 고유 검증 (직접 추가) ───────────────────────────────
const read = (p) => existsSync(p) ? readFileSync(p, 'utf-8') : null

const statsTs = read('src/commands/stats.ts')
must(statsTs?.includes('export function calcAutonomyStats'), 'calcAutonomyStats export')
...
must(koTs?.includes('autonomyNoData'), 'ko.ts stats.autonomyNoData')

const test = read('tests/autonomy-stats.test.ts')
must(existsSync('tests/autonomy-stats.test.ts'), 'tests/autonomy-stats.test.ts 존재')
must(/completionRate/.test(test ?? '') && /null/.test(test ?? ''), '표본 0 → null 테스트')
must(/calcAutonomyStats/.test(test ?? ''), 'calcAutonomyStats 테스트')

if (pass) { console.log('✅ goal 104 gate passes'); process.exit(0) }
console.log('❌ goal 104 gate failed'); process.exit(1)
