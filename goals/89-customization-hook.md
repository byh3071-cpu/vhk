...
- [x] **체감 검증(수동, 필수):** 실제 임시 프로젝트에 `vhk init` 돌리고 Claude Code를 열어 SessionStart 넛지가 실제로 뜨는지, 인터뷰가 자연스러운지 확인 — **완료(2026-07-03, 실제 `claude -p` 세션 스폰으로 실증).** 아래 상세 참조.
- [x] 공통 게이트(_meta) 통과 — `check-goal-89.mjs` 고유 검증 채움(DONE 전환에 따라 M.6 비스텁 요구 충족)
...
## 체감 검증 완료 — 실제 `claude -p` 세션 스폰으로 실증 (2026-07-03)

문서 대조·산출물 실측만으로는 "실제 세션에서 뜨는가"를 증명 못 한다는 한계가 있어, 스크래치 폴더에 최신 수정 반영된 `vhk init -y`를 돌린 뒤 그 폴더에서 **실제 `claude -p` 세션을 스폰**해 검증했다(사용자 승인 하 진행).

**결정적 증거 — `--debug-file` 로그**:
```
Hook SessionStart (node "${CLAUDE_PROJECT_DIR}/.vhk/hooks/customization-check.mjs") provided additionalContext (670 chars)
```
- `${CLAUDE_PROJECT_DIR}`가 정상 치환돼 훅이 성공적으로 실행됨(치환 실패였다면 `node`가 파일을 못 찾아 실패했을 것 — "success"로 기록됨은 치환이 실제로 됐다는 직접 증거).
- `matcher: "startup"`(파이프 아닌 단일값)이 정상 발동함(`Hook SessionStart:startup (SessionStart) success`).
- additionalContext 길이(670자)가 훅 스크립트가 실제로 만드는 지시문 길이와 일치.

**모델 응답 실측**: "지금 세션 시작할 때 특별한 지시사항 받았어?"라고 물었더니, 모델이 훅이 주입한 지시문 내용(4가지 파악 항목·`RULES.md`의 `## 도메인 규칙` 섹션·`vhk sync` 실행·`customization-done` 마커)을 정확히 요약해 돌려주고, "인터뷰 지금 시작할까, 건너뛸까?"라고 A/B 선택지까지 자연스럽게 제시함 — 훅의 지시문이 의도한 대로 정확히 작동함을 실측 확인.

**부수 관찰(참고, vhk 범위 밖)**: 같은 로그에서 `yohan-core` 플러그인의 `context-hint.ps1` 훅이 한글 텍스트를 깨진 인코딩(mojibake)으로 출력하는 게 관찰됨 — vhk와 무관한 별개 플러그인의 별도 버그, 이 조사에서 다루지 않음(참고용 기록만).

이로써 goal 89가 자체적으로 "필수"라 못박았던 마지막 검증 항목이 실제로 완료됐다 — status `IN_PROGRESS` → `DONE` 전환.
