...
leads_to: 이슈 #373의 로깅 스키마+커맨드 부분 완료(이슈 자체는 OPEN 유지) — vhk-auto 자율
  루프의 완주율(런 시작 대비 사람개입 없는 종결)을 실측 데이터로 축적 시작. 이슈가 요구한
  분석 산출(완주율 계산·실패유형 분해·표본 N)과 승격임계 정의는 이번 스코프 밖 — 데이터가
  쌓인 뒤 vhk stats 연동으로 후속 처리
---
