...
export const ko = {
  ...
  bootstrapCursor: {
    ...
    stepSkills: '[6/6] 공통 Agent Skills 확인',
    stepVerify: '배선 검증 — vhk verify',
    skillsCreated: (names: string[]) => `공통 Agent Skills 생성: ${names.join(', ')}`,
    skillsUpdated: (names: string[]) => `공통 Agent Skills 안전 갱신: ${names.join(', ')}`,
    skillsSkipped: (names: string[]) => `공통 Agent Skills 최신 상태: ${names.join(', ')}`,
    skillsOutdated: (names: string[]) =>
      `사용자 수정 또는 미상 파일로 보존한 Agent Skills: ${names.join(', ')} — 정본과 수동 병합하세요.`,
  ...
  sync: {
    ...
    skillCreated: (p: string) => `✓ ${p} — 공통 Agent Skill 생성`,
    skillUpdated: (p: string) => `✓ ${p} — 알려진 관리본 갱신`,
    skillConflict: (p: string) => `⚠ ${p} — 사용자 수정 또는 미상 파일로 보존 (수동 병합 필요)`,
    skillConflictSummary: (n: number) =>
      n === 0 ? '✅ Agent Skill 보존 충돌 0건' : `❌ Agent Skill 보존 충돌 ${n}건`,
