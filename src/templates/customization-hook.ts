import { formatStackStatusNote } from '../lib/stack-state.js'

// 커스터마이징 인터뷰 넛지 훅(배출 스크립트). 인터뷰 페이로드는 도구 무관 SoT 1개,
// 출력 직렬화만 `--format` 으로 분기(RFC 0057 §7 — 트리거 계층 에이전트 불가지론화):
//   기본(플래그 없음) = Claude Code SessionStart `hookSpecificOutput`(하위호환 — 기존 배선 무변)
//   --format cursor   = Cursor `sessionStart` `additional_context`(공식 스키마: cursor.com/docs/agent/hooks)
// 마커 감지(.vhk/NEEDS_CUSTOMIZATION·customization-done)는 도구 무관 — 어떤 에이전트가 실행해도 동일.
export function CUSTOMIZATION_HOOK_TEMPLATE(): string {
  return [
    "import { existsSync, readFileSync } from 'node:fs'",
    "import { join } from 'node:path'",
    '',
    `const STACK_CANDIDATE_NOTE = ${JSON.stringify(formatStackStatusNote('candidate'))}`,
    '',
    ...
    "]",
    '',
    'const STACK_INTERVIEW_LINES = [',
    "  '',",
    "  '[0단계 — 기술 스택] RULES.md의 기술 스택은 후보 상태다. 다른 질문보다 먼저 실제 사용할 기술 스택을 하나씩 확인해라.',",
    "  '사용자 확인을 받으면 RULES.md의 기술 스택 목록을 고치고 상태를 `확정`으로 바꿔라. 추측으로 확정하지 마라.',",
    ']',
    '',
    'const DOMAIN_INTERVIEW_LINES = [',
    "  '',",
    ...
    "      existsSync(rulesPath) && readFileSync(rulesPath, 'utf-8').includes(STACK_CANDIDATE_NOTE)",
    '    const additionalContext = [',
    '      ...INTRO_LINES,',
    '      ...(stackCandidate ? STACK_INTERVIEW_LINES : []),',
    '      ...DOMAIN_INTERVIEW_LINES,',
    "    ].join('\\n')",
    ...
    '  }',
    '} catch {',
    '  // fail-open: 어떤 내부 오류도 세션 시작을 막지 않는다.',
    '  // 다만 조용히 삼키지는 않는다 — 경로·원문 오류 없이 고정 문구만 stderr 로 알린다',
    '  // (stdout 은 훅 JSON 계약이라 오염 금지).',
    "  process.stderr.write('[vhk] 커스터마이즈 안내를 건너뜁니다 — RULES.md 확인에 실패했습니다.\\n')",
    '}',
    'process.exit(0)',
    '',
  ].join('\n');
