...
export function buildStartPrompt(git: string, goalLine: string): string {
  ...
  return [
    ...
    '3. RULES.md와 프로젝트 문서가 지정한 추적 원본에서 작업 정의·수용 기준을 확인하세요. goals/*.md가 있으면 로컬 실행 상태로 참고하고, 카드가 없으면 상태를 unknown으로 두세요. scripts/check-goal-<번호>.mjs는 직접 수정하지 말고 vhk goal sync로 재생성하세요. 블로커가 있으면 가장 먼저 언급하세요.',
    '',
...
export function buildHandoffPrompt(
  ...
  return [
    ...
    "2. 테스트를 실행했는지/안 했는지, 결과가 어땠는지 docs/devlog/YYYY-MM-DD-<작업명>.md 에 기록해 주세요. (안 돌렸으면 '미실행'. dev log 는 append-only — 추가만, 과거 항목 수정·삭제 금지)",
