...
import { configSetrulesRoot } from './config.js'

// goal 92 — `vhk config set-rules-root <path>` 저장 직후 즉시 loadCoreRuleset() 재호출해
...
describe('configSetrulesRoot', () => {
  ...
  beforeEach(() => {
    ...
    // critic 지적(M1): PRIVATE_RULES_ROOT 가 테스트 실행 셸에 실제로 설정돼 있으면
  ...
  it('유효한 core-ruleset.yaml 이 있는 경로 → 저장 + 즉시 live 확인 메시지', async () => {
    ...
    await configSetrulesRoot(brainDir, home)

    expect(readHomeConfig(home)).toEqual({ rulesRoot: brainDir })
  ...
  it('경로는 저장되지만 그 경로에 core-ruleset.yaml 이 없으면 정직하게 경고', async () => {
    ...
    await configSetrulesRoot(emptyDir, home)

    // 저장 자체는 사용자가 명시한 그대로 이뤄진다(경로가 지금 비어있어도 나중에 채울 수 있음).
    expect(readHomeConfig(home)).toEqual({ rulesRoot: emptyDir })
    const joined = logs.join('\n')
  ...
  // critic 지적(M2): PRIVATE_RULES_ROOT 가 다른 유효 경로로 설정돼 있으면, 방금 저장한 경로가
  // 유효해도 지금 당장은 env 가 우선이라 안 쓰인다 — "성공"이라고만 말하면 사용자가 속는다.
  it('PRIVATE_RULES_ROOT 가 다른 경로를 가리키면 "지금은 안 쓰인다"고 정직하게 경고', async () => {
    const home = tmpHome()
    ...
    await configSetrulesRoot(newBrain, home)

    expect(readHomeConfig(home)).toEqual({ rulesRoot: newBrain }) // 저장 자체는 정상
    const joined = logs.join('\n')
    expect(joined).toMatch(/안 쓰|우선 적용|PRIVATE_RULES_ROOT/) // "성공했다"고 오도하지 않음
  ...
  // critic 재검증(2026-07-03, main 병합 후) 발견: rulesRootPath 를 path.resolve 없이 그대로
  ...
  it('상대경로로 저장해도 절대경로로 정규화되어 이후 다른 cwd 에서도 안 깨짐', async () => {
    ...
    try {
      ...
      await configSetrulesRoot(brainBasename, home)
    } finally {
      process.chdir(originalCwd)
    }

    const saved = readHomeConfig(home)
    expect(path.isAbsolute(saved?.rulesRoot ?? '')).toBe(true)
