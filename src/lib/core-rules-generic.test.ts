...
describe('범용 규칙 파일 계약', () => {
  ...
  const originalLegacyRoot = process.env.PRIVATE_RULES_ROOT

  afterEach(() => {
    if (originalRulesFile === undefined) delete process.env.VHK_RULES_FILE
    else process.env.VHK_RULES_FILE = originalRulesFile
    if (originalLegacyRoot === undefined) delete process.env.PRIVATE_RULES_ROOT
    else process.env.PRIVATE_RULES_ROOT = originalLegacyRoot
  })
  ...
  it('VHK_RULES_FILE이 홈 설정과 legacy보다 우선한다', () => {
    ...
    writeHomeConfig({ rulesFile: homeFile, rulesRoot: files }, home)
    process.env.VHK_RULES_FILE = envFile
    process.env.PRIVATE_RULES_ROOT = files
  ...
  it('홈 rulesFile을 직접 읽고 공개 마커에는 개인 저장소명이 없다', () => {
    ...
    expect(rendered).not.toContain('private-rules-repository')
  })
  ...
  it('legacy root는 한 릴리스 동안 동작하며 제거 예정 경고를 남긴다', () => {
    ...
    delete process.env.VHK_RULES_FILE
    process.env.PRIVATE_RULES_ROOT = root
